...
/**
 * Parse invoice data from extracted text using AI
 */
export async function parseInvoiceWithAI(pdfText: string): Promise<PDFAnalysisResult> {
    try {
...
export async function validateInvoiceData(invoiceData: any): Promise<{
    ...
    // Validate GST calculation (18% standard rate)
...
/**
 * Generate AI-powered compliance report for extracted invoice
 */
export async function generateComplianceReport(invoiceData: any, extractedText: string): Promise<string> {
    try {
 ...
 */
export async function analyzePDF(pdfBuffer: Buffer): Promise<{
    success: boolean;
